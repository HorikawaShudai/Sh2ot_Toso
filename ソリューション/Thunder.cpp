...
//マクロ定義
#define THUNDER_COUNTER (400)		//雷が鳴るカウント
...
void InitThunder(void)
{
	...
	g_Thunder.pos = D3DXVECTOR3(100.0f, +0.0f, 1000.0f);
	g_ThunderCnt = THUNDER_COUNTER;
...
void UpdateThunder(void)
{
	...
	if (bRain == true)
	{
		if (g_Thunder.bUse == false)
		{
			g_Thunder.nCnt++;

			if (g_Thunder.nCnt%THUNDER_COUNTER == 0)
			{
				g_Thunder.col = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
				g_Thunder.bUse = true;
			}
		}
		else if (g_Thunder.bUse == true)
		{
			g_Thunder.col.a -= 0.01f;
			if (g_Thunder.col.a < 0.0f)
			{
				g_Thunder.bUse = false;
				g_ThunderCnt = rand()%200+THUNDER_COUNTER;
			}
		}
